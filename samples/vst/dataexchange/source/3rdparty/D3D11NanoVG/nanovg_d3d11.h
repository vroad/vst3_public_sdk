...
// Flag indicating if geoemtry based anti-aliasing is used (may not be needed
// when using MSAA).
#define NVG_ANTIALIAS 1

// Flag indicating if strokes should be drawn using stencil buffer. The
// rendering will be a little slower, but path overlaps (i.e. self-intersecting
...
int nvd3dCreateImageFromHandle(struct NVGcontext *ctx, void *texture, int w,
                               int h, int flags);
...
// There's probably a better way... (but note we can't use the
// IInterace_Method() helpers because They won't work when compiled for cpp)
#ifdef __cplusplus
...
#define D3D_API_4(p, name, arg1, arg2, arg3, arg4)                             \
  p->name(arg1, arg2, arg3, arg4)
#define D3D_API_5(p, name, arg1, arg2, arg3, arg4, arg5)                       \
  p->name(arg1, arg2, arg3, arg4, arg5)
#define D3D_API_6(p, name, arg1, arg2, arg3, arg4, arg5, arg6)                 \
  p->name(arg1, arg2, arg3, arg4, arg5, arg6)
...
#define D3D_API_3(p, name, arg1, arg2, arg3)                                   \
  p->lpVtbl->name(p, arg1, arg2, arg3)
#define D3D_API_4(p, name, arg1, arg2, arg3, arg4)                             \
  p->lpVtbl->name(p, arg1, arg2, arg3, arg4)
#define D3D_API_5(p, name, arg1, arg2, arg3, arg4, arg5)                       \
  p->lpVtbl->name(p, arg1, arg2, arg3, arg4, arg5)
#define D3D_API_6(p, name, arg1, arg2, arg3, arg4, arg5, arg6)                 \
  p->lpVtbl->name(p, arg1, arg2, arg3, arg4, arg5, arg6)
#define D3D_API_RELEASE(p)                                                     \
  {                                                                            \
    if ((p)) {                                                                 \
      (p)->lpVtbl->Release((p));                                               \
...
static struct D3DNVGtexture *D3Dnvg__allocTexture(struct D3DNVGcontext *D3D) {
  ...
  if (tex == NULL) {
    if (D3D->ntextures + 1 > D3D->ctextures) {
      struct D3DNVGtexture *textures;
      int ctextures = D3Dnvg__maxi(D3D->ntextures + 1, 4) +
                      D3D->ctextures / 2; // 1.5x Overallocate
      textures = (struct D3DNVGtexture *)realloc(
          D3D->textures, sizeof(struct D3DNVGtexture) * ctextures);
      if (textures == NULL)
        return NULL;
...
static struct D3DNVGtexture *D3Dnvg__findTexture(struct D3DNVGcontext *D3D,
                                                 int id) {
  int i;
  for (i = 0; i < D3D->ntextures; i++)
  ...
  for (i = 0; i < D3D->ntextures; i++) {
    if (D3D->textures[i].id == id) {
      if (D3D->textures[i].tex != 0 &&
          (D3D->textures[i].flags & NVG_IMAGE_NODELETE) == 0) {
...
static int D3Dnvg__createShader(struct D3DNVGcontext *D3D,
                                struct D3DNVGshader *shader,
                                const void *vshader, unsigned int vshader_size,
                                const void *fshader,
                                unsigned int fshader_size) {
  ...
  hr = D3D_API_4(D3D->pDevice, CreateVertexShader, vshader, vshader_size, NULL,
                 &vert);
  hr = D3D_API_4(D3D->pDevice, CreatePixelShader, fshader, fshader_size, NULL,
                 &frag);
...
void D3Dnvg_buildFanIndices(struct D3DNVGcontext *D3D) {
  ...
  D3D_API_5(D3D->pDeviceContext, Map, (ID3D11Resource *)D3D->pFanIndexBuffer, 0,
  ...
  D3D_API_2(D3D->pDeviceContext, Unmap, (ID3D11Resource *)D3D->pFanIndexBuffer,
            0);
}

struct NVGvertex *D3Dnvg_beginVertexBuffer(struct D3DNVGcontext *D3D,
  ...
  if ((D3D->VertexBuffer.CurrentBufferEntry + maxCount) >=
      D3D->VertexBuffer.MaxBufferEntries) {
    *baseOffset = 0;
    D3D->VertexBuffer.CurrentBufferEntry = maxCount;
    D3D_API_5(D3D->pDeviceContext, Map,
              (ID3D11Resource *)D3D->VertexBuffer.pBuffer, 0,
              D3D11_MAP_WRITE_DISCARD, 0, &resource);
  } else {
...
void D3Dnvg_endVertexBuffer(struct D3DNVGcontext *D3D) {
  D3D_API_2(D3D->pDeviceContext, Unmap,
            (ID3D11Resource *)D3D->VertexBuffer.pBuffer, 0);
}

static void D3Dnvg__copyVerts(struct NVGvertex *pDest,
                              const struct NVGvertex *pSource,
                              unsigned int num) {
  unsigned int i;
  for (i = 0; i < num; i++) {
...
static unsigned int D3Dnvg_updateVertexBuffer(ID3D11DeviceContext *pContext,
                                              struct D3DNVGBuffer *buffer,
  ...
  if ((buffer->CurrentBufferEntry + nverts) >= buffer->MaxBufferEntries) {
    ...
    D3D_API_5(pContext, Map, (ID3D11Resource *)buffer->pBuffer, 0,
              D3D11_MAP_WRITE_DISCARD, 0, &resource);
  } else {
    D3D_API_5(pContext, Map, (ID3D11Resource *)buffer->pBuffer, 0,
              D3D11_MAP_WRITE_NO_OVERWRITE, 0, &resource);
  ...
  D3Dnvg__copyVerts(
      (((struct NVGvertex *)resource.pData) + buffer->CurrentBufferEntry),
      (const struct NVGvertex *)verts, nverts);
...
static void D3Dnvg_setBuffers(struct D3DNVGcontext *D3D,
  ...
  D3D_API_3(D3D->pDeviceContext, IASetIndexBuffer, D3D->pFanIndexBuffer,
            DXGI_FORMAT_R32_UINT, 0);
  D3D_API_5(D3D->pDeviceContext, IASetVertexBuffers, 0, 1, pBuffers, strides,
            offsets);
...
static int D3Dnvg__renderCreate(void *uptr) {
  ...
  const D3D11_DEPTH_STENCILOP_DESC frontOp = {
      D3D11_STENCIL_OP_KEEP, D3D11_STENCIL_OP_KEEP, D3D11_STENCIL_OP_INCR,
      D3D11_COMPARISON_ALWAYS};
  const D3D11_DEPTH_STENCILOP_DESC backOp = {
      D3D11_STENCIL_OP_KEEP, D3D11_STENCIL_OP_KEEP, D3D11_STENCIL_OP_DECR,
      D3D11_COMPARISON_ALWAYS};

  const D3D11_DEPTH_STENCILOP_DESC aaOp = {
      D3D11_STENCIL_OP_KEEP, D3D11_STENCIL_OP_KEEP, D3D11_STENCIL_OP_KEEP,
      D3D11_COMPARISON_EQUAL};
  const D3D11_DEPTH_STENCILOP_DESC fillOp = {
      D3D11_STENCIL_OP_ZERO, D3D11_STENCIL_OP_ZERO, D3D11_STENCIL_OP_ZERO,
  ...
  if (D3D->flags & NVG_ANTIALIAS) {
    if (D3Dnvg__createShader(D3D, &D3D->shader, g_D3D11VertexShader_Main,
                             sizeof(g_D3D11VertexShader_Main),
                             g_D3D11PixelShaderAA_Main,
                             sizeof(g_D3D11PixelShaderAA_Main)) == 0)
      return 0;
  } else {
    if (D3Dnvg__createShader(D3D, &D3D->shader, g_D3D11VertexShader_Main,
                             sizeof(g_D3D11VertexShader_Main),
                             g_D3D11PixelShader_Main,
                             sizeof(g_D3D11PixelShader_Main)) == 0)
      return 0;
  }
  ...
  bufferDesc.ByteWidth =
      sizeof(struct NVGvertex) * D3D->VertexBuffer.MaxBufferEntries;
  hr = D3D_API_3(D3D->pDevice, CreateBuffer, &bufferDesc, NULL,
                 &D3D->VertexBuffer.pBuffer);
  ...
  hr = D3D_API_5(D3D->pDevice, CreateInputLayout, LayoutRenderTriangles, 2,
                 g_D3D11VertexShader_Main, sizeof(g_D3D11VertexShader_Main),
                 &D3D->pLayoutRenderTriangles);
  ...
  hr = D3D_API_2(D3D->pDevice, CreateRasterizerState, &rasterDesc,
                 &D3D->pRSNoCull);
  ...
  // No color write
  hr = D3D_API_2(D3D->pDevice, CreateDepthStencilState, &depthStencilDesc,
                 &D3D->pDepthStencilDrawShapes);
  ...
  D3D_API_2(D3D->pDevice, CreateSamplerState, &sampDesc,
            &D3D->pSamplerState[0]);
...
static int D3Dnvg__renderCreateTexture(void *uptr, int type, int w, int h,
                                       int imageFlags,
  ...
  if (data != NULL) {
    D3D_API_6(D3D->pDeviceContext, UpdateSubresource,
              (ID3D11Resource *)tex->tex, 0, NULL, data,
              tex->width * pixelWidthBytes,
              (tex->width * tex->height) * pixelWidthBytes);
  }
  ...
  D3D_API_3(D3D->pDevice, CreateShaderResourceView, (ID3D11Resource *)tex->tex,
            &viewDesc, &tex->resourceView);
...
static int D3Dnvg__renderUpdateTexture(void *uptr, int image, int x, int y,
                                       int w, int h,
                                       const unsigned char *data) {
  struct D3DNVGcontext *D3D = (struct D3DNVGcontext *)uptr;
  ...
  pData = (unsigned char *)data + (y * (tex->width * pixelWidthBytes)) +
          (x * pixelWidthBytes);
  D3D_API_6(D3D->pDeviceContext, UpdateSubresource, (ID3D11Resource *)tex->tex,
            0, &box, pData, tex->width, tex->width * tex->height);
...
static int D3Dnvg__convertPaint(struct D3DNVGcontext *D3D,
                                struct D3DNVGfragUniforms *frag,
                                struct NVGpaint *paint,
                                struct NVGscissor *scissor, float width,
...
static struct D3DNVGfragUniforms *nvg__fragUniformPtr(struct D3DNVGcontext *D3D,
                                                      int i);

static void D3Dnvg__setUniforms(struct D3DNVGcontext *D3D, int uniformOffset,
                                int image) {
  ...
  D3D_API_5(D3D->pDeviceContext, Map, (ID3D11Resource *)D3D->pPSConstants, 0,
  ...
  if (image != 0) {
    struct D3DNVGtexture *tex = D3Dnvg__findTexture(D3D, image);
    if (tex != NULL) {
      D3D_API_3(D3D->pDeviceContext, PSSetShaderResources, 0, 1,
                &tex->resourceView);
    } else {
...
static void D3Dnvg__renderViewport(void *uptr, float width, float height,
  ...
  D3D_API_5(D3D->pDeviceContext, Map, (ID3D11Resource *)D3D->pVSConstants, 0,
            D3D11_MAP_WRITE_DISCARD, 0, &mappedResource);
...
static void D3Dnvg__fill(struct D3DNVGcontext *D3D, struct D3DNVGcall *call) {
  ...
  // Draw shapes
  D3D_API_2(D3D->pDeviceContext, OMSetDepthStencilState,
            D3D->pDepthStencilDrawShapes, 0);
  D3D_API_3(D3D->pDeviceContext, OMSetBlendState, D3D->pBSNoWrite, NULL,
            0xFFFFFFFF);
  ...
  D3D_API_1(D3D->pDeviceContext, IASetPrimitiveTopology,
  ...
  for (i = 0; i < npaths; i++) {
    ...
    if (numIndices < D3D->VertexBuffer.MaxBufferEntries) {
      D3D_API_3(D3D->pDeviceContext, DrawIndexed, numIndices, 0,
                paths[i].fillOffset);
    }
  }
  ...
  D3D_API_3(D3D->pDeviceContext, OMSetBlendState, D3D->pBSBlend, NULL,
            0xFFFFFFFF);
  ...
  if (D3D->flags & NVG_ANTIALIAS) {
    D3D_API_2(D3D->pDeviceContext, OMSetDepthStencilState,
              D3D->pDepthStencilDrawAA, 0);
    // Draw fringes
    for (i = 0; i < npaths; i++) {
      D3D_API_2(D3D->pDeviceContext, Draw, paths[i].strokeCount,
  ...
  D3D_API_2(D3D->pDeviceContext, OMSetDepthStencilState, D3D->pDepthStencilFill,
            0);
  D3D_API_2(D3D->pDeviceContext, Draw, call->triangleCount,
            call->triangleOffset);

  D3D_API_2(D3D->pDeviceContext, OMSetDepthStencilState,
            D3D->pDepthStencilDefault, 0);
}

static void D3Dnvg__convexFill(struct D3DNVGcontext *D3D,
                               struct D3DNVGcall *call) {
  struct D3DNVGpath *paths = &D3D->paths[call->pathOffset];
  int i, npaths = call->pathCount;
  ...
  D3D_API_1(D3D->pDeviceContext, IASetPrimitiveTopology,
            D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
  for (i = 0; i < npaths; i++) {
    // Draws a fan using indices to fake it up, since there isn't a fan
    // primitive in D3D11.
...
static void D3Dnvg__triangles(struct D3DNVGcontext *D3D,
                              struct D3DNVGcall *call) {
  D3D_API_1(D3D->pDeviceContext, IASetPrimitiveTopology,
            D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
...
static void D3Dnvg__renderFlush(void *uptr) {
  ...
  if (D3D->ncalls > 0) {
    unsigned int buffer0Offset = D3Dnvg_updateVertexBuffer(
        D3D->pDeviceContext, &D3D->VertexBuffer, D3D->verts, D3D->nverts);
    D3Dnvg_setBuffers(D3D, buffer0Offset);

    // Ensure valid state
    D3D_API_3(D3D->pDeviceContext, PSSetConstantBuffers, 0, 1,
              &D3D->pPSConstants);
    D3D_API_3(D3D->pDeviceContext, VSSetConstantBuffers, 0, 1,
              &D3D->pVSConstants);
    ...
    for (i = 0; i < D3D->ncalls; i++) {
      ...
      if (call->image != 0) {
        ...
        if (tex != NULL) {
          D3D_API_3(
              D3D->pDeviceContext, PSSetSamplers, 0, 1,
              &D3D->pSamplerState[(tex->flags & NVG_IMAGE_REPEATX ? 1 : 0) +
                                  (tex->flags & NVG_IMAGE_REPEATY ? 2 : 0)]);
        }
      }
...
static struct D3DNVGcall *D3Dnvg__allocCall(struct D3DNVGcontext *D3D) {
  ...
  if (D3D->ncalls + 1 > D3D->ccalls) {
    struct D3DNVGcall *calls;
    int ccalls = D3Dnvg__maxi(D3D->ncalls + 1, 128) +
                 D3D->ccalls / 2; // 1.5x Overallocate
    calls = (struct D3DNVGcall *)realloc(D3D->calls,
                                         sizeof(struct D3DNVGcall) * ccalls);
    if (calls == NULL)
      return NULL;
    D3D->calls = calls;
    D3D->ccalls = ccalls;
...
static int D3Dnvg__allocPaths(struct D3DNVGcontext *D3D, int n) {
  ...
  if (D3D->npaths + n > D3D->cpaths) {
    struct D3DNVGpath *paths;
    int cpaths = D3Dnvg__maxi(D3D->npaths + n, 128) +
                 D3D->cpaths / 2; // 1.5x Overallocate
    paths = (struct D3DNVGpath *)realloc(D3D->paths,
                                         sizeof(struct D3DNVGpath) * cpaths);
    if (paths == NULL)
      return -1;
    D3D->paths = paths;
    D3D->cpaths = cpaths;
...
static int D3Dnvg__allocVerts(struct D3DNVGcontext *D3D, int n) {
  ...
  if (D3D->nverts + n > D3D->cverts) {
    struct NVGvertex *verts;
    int cverts = D3Dnvg__maxi(D3D->nverts + n, 4096) +
                 D3D->cverts / 2; // 1.5x Overallocate
    verts = (struct NVGvertex *)realloc(D3D->verts,
                                        sizeof(struct NVGvertex) * cverts);
    if (verts == NULL)
      return -1;
    D3D->verts = verts;
    D3D->cverts = cverts;
...
static int D3Dnvg__allocFragUniforms(struct D3DNVGcontext *D3D, int n) {
  ...
  if (D3D->nuniforms + n > D3D->cuniforms) {
    unsigned char *uniforms;
    int cuniforms = D3Dnvg__maxi(D3D->nuniforms + n, 128) +
                    D3D->cuniforms / 2; // 1.5x Overallocate
...
static void D3Dnvg__vset(struct NVGvertex *vtx, float x, float y, float u,
                         float v) {
  vtx->x = x;
  vtx->y = y;
  vtx->u = u;
...
static void D3Dnvg__renderFill(void *uptr, struct NVGpaint *paint,
                               NVGcompositeOperationState compositeOperation,
                               struct NVGscissor *scissor, float fringe,
                               const float *bounds, const struct NVGpath *paths,
                               int npaths) {
  struct D3DNVGcontext *D3D = (struct D3DNVGcontext *)uptr;
  ...
  for (i = 0; i < npaths; i++) {
    ...
    if (path->nfill > 0) {
      ...
      memcpy(&D3D->verts[offset], path->fill,
             sizeof(struct NVGvertex) * path->nfill);
      offset += path->nfill;
    }
    if (path->nstroke > 0) {
      copy->strokeOffset = offset;
      copy->strokeCount = path->nstroke;
      memcpy(&D3D->verts[offset], path->stroke,
             sizeof(struct NVGvertex) * path->nstroke);
      offset += path->nstroke;
    }
  }
  ...
  if (call->type == D3DNVG_FILL) {
    ...
    // Fill shader
    D3Dnvg__convertPaint(
        D3D, nvg__fragUniformPtr(D3D, call->uniformOffset + D3D->fragSize),
    ...
    if (call->uniformOffset == -1)
      goto error;
    // Fill shader
    D3Dnvg__convertPaint(D3D, nvg__fragUniformPtr(D3D, call->uniformOffset),
                         paint, scissor, fringe, fringe, -1.0f);
  }
...
static void D3Dnvg__renderStroke(void *uptr, struct NVGpaint *paint,
                                 NVGcompositeOperationState compositeOperation,
                                 struct NVGscissor *scissor, float fringe,
                                 float strokeWidth, const struct NVGpath *paths,
                                 int npaths) {
  struct D3DNVGcontext *D3D = (struct D3DNVGcontext *)uptr;
  struct D3DNVGcall *call = D3Dnvg__allocCall(D3D);
  int i, maxverts, offset;
...
static void D3Dnvg__renderTriangles(
    void *uptr, struct NVGpaint *paint,
    NVGcompositeOperationState compositeOperation, struct NVGscissor *scissor,
  ...
  memcpy(&D3D->verts[call->triangleOffset], verts,
         sizeof(struct NVGvertex) * nverts);
...
struct NVGcontext *nvgCreateD3D11(ID3D11Device *pDevice, int flags) {
  ...
  struct D3DNVGcontext *D3D =
      (struct D3DNVGcontext *)malloc(sizeof(struct D3DNVGcontext));
  if (D3D == NULL) {
    goto error;
  }
...
void nvgDeleteD3D11(struct NVGcontext *ctx) { nvgDeleteInternal(ctx); }

#ifdef IMPLEMENTED_IMAGE_FUNCS
int nvd3dCreateImageFromHandle(struct NVGcontext *ctx, void *textureId, int w,
                               int h, int flags) {

  /*struct D3DNVGcontext* gl = (struct
  D3DNVGcontext*)nvgInternalParams(ctx)->userPtr; struct D3DNVGtexture* tex =
  D3Dnvg__allocTexture(gl);
...
unsigned int nvd3dImageHandle(struct NVGcontext *ctx, int image) {
  ...
  struct D3DNVGcontext* gl = (struct
  D3DNVGcontext*)nvgInternalParams(ctx)->userPtr; struct D3DNVGtexture* tex =
  D3Dnvg__findTexture(gl, image); return tex->tex;*/
  return 0;
}
...
void nvd3dImageFlags(struct NVGcontext *ctx, int image, int flags) {
  ...
  struct D3DNVGcontext* gl = (struct
  D3DNVGcontext*)nvgInternalParams(ctx)->userPtr; struct D3DNVGtexture* tex =
  D3Dnvg__findTexture(gl, image); tex->flags = flags;
  */
}
#endif
